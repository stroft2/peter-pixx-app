      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {presets.map(preset => (
          <button
            ...
            disabled={isLoading}
            className={`w-full text-center bg-purple-900/40 border border-transparent text-purple-200 font-semibold py-3 px-4 rounded-md transition-all duration-200 ease-in-out hover:bg-purple-800/60 hover:border-purple-500/30 active:scale-95 text-sm disabled:opacity-50 disabled:cursor-not-allowed ${selectedPresetPrompt === preset.prompt ? 'ring-2 ring-offset-2 ring-offset-purple-950 ring-purple-500 bg-purple-800/60' : ''}`}
          >
            {preset.name}
          </button>
        ))}
